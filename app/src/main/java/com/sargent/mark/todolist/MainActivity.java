...
public class MainActivity extends AppCompatActivity implements AddToDoFragment.OnDialogCloseListener, UpdateToDoFragment.OnUpdateDialogCloseListener{
    ...
    protected void onStart() {
        ...
        adapter = new ToDoListAdapter(cursor, new ToDoListAdapter.ItemClickListener() {
            ...
            // category and status params added
            @Override
    ...
    // here i can get items by category
    ...
    // added method to update checkbox's status
    ...
    // items selected and show in view
    @Override
    public boolean onOptionsItemSelected(MenuItem item) {
        int id = item.getItemId();
        switch (id) {
            case R.id.action_dropdown0:
                cursor = getAllItems(db);
                adapter.swapCursor(cursor);
                return true;
            case R.id.action_dropdown1:
                cursor = getCategoryItems(db, getString(R.string.category_1));
                adapter.swapCursor(cursor);
                return true;
            case R.id.action_dropdown2:
                cursor = getCategoryItems(db, getString(R.string.category_2));
                adapter.swapCursor(cursor);
                return true;
            case R.id.action_dropdown3:
                cursor = getCategoryItems(db, getString(R.string.category_3));
                adapter.swapCursor(cursor);
                return true;
            case R.id.action_dropdown4:
                cursor = getCategoryItems(db, getString(R.string.category_4));
                adapter.swapCursor(cursor);
                return true;
            default:
